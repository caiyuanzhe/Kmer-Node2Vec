...
class KMerEmbeddings:
    ...
    def train(self):
        """ Obtain k-mer embeddings. """
        clf = KMerNode2Vec(p=self.p, q=self.q, workers=self.workers)
        clf.fit(
            graph=nx.DiGraph(),
            seqs=self.seqs,
            mer=self.mer,
            path_to_edg_list_file=self.kmer_vec_output_dir+f"networkfile-p{self.p}-q{self.q}-{arrow.utcnow().format('YYYYMMDD-HHmm')}.edg",
            path_to_embeddings_file=self.kmer_vec_output_dir+f"kmer-node2vec-p{self.p}-q{self.q}-{arrow.utcnow().format('YYYYMMDD-HHmm')}.txt",
        )
...
class SequenceEmbeddings:
    ...
    def segment_embeddings(self):
        ...
        for clf in clfs:
            ...
            clf.save_embs_format(
                ...
                f"{'SegmentVectors'}-{self.kmer2vec_file.split('/')[-1].split('.txt')[0]}"
            )
            print('******')
    ...
    def subseg_embeddings(self):
        ...
        for clf in clfs:
            ...
            clf.save_embs_format(
                self.seg_vec_output_dir,
                f"{'SubSegmentVectors'}-{self.kmer2vec_file.split('/')[-1].split('.txt')[0]}"
            )
...
class SequenceRetrieval:
    ...
    def train(
        ...
        """ Print the Top-KN result for sequence retrieval task """
...
def sequence_embeddings(work_dir):
    clf = SequenceEmbeddings(
        ...
        kmer2vec_file=work_dir+'kmer-node2vec-p1.0-q0.001-20220421-0754.txt',
        seq_dir=work_dir,
        segment_length=10,
        segment_number=10,
...
def sequence_retrieval(work_dir):
    clf = SequenceRetrieval(
        ...
        subsegment_vec_file=work_dir+'extracted_sub_segment.txt',
        faiss_index_path=work_dir+'faiss-index-file',
        faiss_log=work_dir+'faiss-result.log',
        top_kn=20,
    )
    clf.train(
        dimension=128,  # consistent with dimensionality of sequence embeddings
        index_method='HNSW',
...
def pipeline():
    kmer_embeddings(work_dir='../data_dir/output/demo/')  # Step1
    sequence_embeddings(work_dir='../data_dir/output/demo/')  # Step2
    sequence_retrieval(work_dir='../data_dir/output/demo/')  # Step3
